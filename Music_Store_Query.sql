...
select billing_city, round(sum(total)) as invoice_total from invoice
...
GROUP BY customer.customer_id, customer.first_name, customer.last_name  -- Added first_name and last_name to GROUP BY
...
select name as track_name  , milliseconds as avg_track_length
from track
where milliseconds > (
